...
mod ERC721 {
    ...
    impl ExternalImpl of ExternalTrait {
        fn mint(ref self: ContractState, to: ContractAddress) {
            ...
            // 新しいtokenIdを生成
            let current_token_id = self._current_token_id.read();
            let new_token_id = current_token_id + 1;

            // 新しいトークンをmint
            self._mint(to, new_token_id);

            self._current_token_id.write(new_token_id);
        }
    }
